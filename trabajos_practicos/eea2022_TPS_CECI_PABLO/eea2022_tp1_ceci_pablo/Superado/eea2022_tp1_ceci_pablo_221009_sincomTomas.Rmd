...
-   Con respecto al valor **estimado** de 1.26 del coeficiente **βgeneroMasculino** nos indica el desplazamiento paralelo del género masculino respecto de la curva de la categoría basal (femenino).
    ...
    Al determinarse el género para el dato observado, si es masculino, tomará como valor binario el número uno. Así, la ordenada al origen de la curva asociada a la categoría basal femenina, **β0**, se transformará en: **β3 = β0** + **βgeneroMasculino**.
...
Ahora observamos que la interacción agrega otro tipo de fenómeno con respecto al modelo binario anterior que solamente incidía en la ordenada al origen dejando paralelas a las curvas según género del individuo. En este caso, la interacción genera un cambio en la pendiente de la curva de los hombres reflejado sobre la variable edad. Es decir, que por cada cambio en una unidad de la edad de una mujer, siempre a ceteris paribus, los masculinos aumentarán en β5 la pendiente con respecto a la femenina con parámetro β2.

El β4 generará un efecto análogo al de la variable binaria por género, solo que en este caso generará un abanico de desplazamientos, de curvas paralelas.
...
**El modelo inicial frente al nuevo dataset se ve significativamente afectado desde el punto de vista del R2 ajustado cayendo su capacidad de explicar la variabilidad observada a solamente el 10.95%. con respecto al del modelo incial de 35.4%**
...
```{r}
pred_modelo_6 = augment(modelo_6, newdata = df_ES_test)
```
...
**Se observa que el modelo inicial empeora su RMSE pasando de 10.21 a 10.62**.
...
**Se observa que el modelo inicial empeora su MAE pasando de 7.58 a 8.15**.
...
```{r}
pred_modelo_robust = augment(modelo_robust, newdata = df_ES_test)
```
...
Como resultado de la aplicación del modelo robusto, se observa la capacidad del modelo robusto frente a un nuevo dataset. Comparando la performance del robusto y el inicial frente al nuevo dataset se observa un RMSE de 10.26 frente al 10.62 obtenido por el modelo inicial. Incluso el modelo robusto es apenas inferior a la performance obtenida por el modelo inicial sobre el data set original, un 0.5% (Vs 10.21).

Además el modelo robusto obtiene un MAE de 7.52 que supera al modelo inicial corrido tanto en el dataset inicial como en el del modelo 6 que arrojaron 7.58 y 8.15 respectivamente.

Finalmente se comprueba que el modelo robusto frente al inicial fue capaz de contener los impactos de los valores atípicos a diferencia del modelo inicial que se encontró considerablemente afectado.
